...
for (int i = 0; i < num; ++i)
{
draw(graph(A[0],A[i+1+off+skip]),s);
}
s = dashed + green + 1.;
draw(graph(A[0],A[1+off+skip+res-1]),s);
s = solid + red + 0.5;
for (int i = 0; i < skip; ++i)
{
draw(graph(A[0],A[i+1+off]),s);
}
s = solid + blue + 0.5;
for (int i = 0; i < skip; ++i)
{
draw(graph(A[0],A[i+1+off+skip+num]),s);
}
