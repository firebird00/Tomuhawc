...
!
  ...
  type (type_equilibrium), pointer, intent(IN)  ::  equilibrium_in(:)
  type (type_equilibrium), pointer, intent(OUT)  ::  equilibrium_out(:)
  ...
  !
  integer :: inequi, nb1D, i, it, i_gridtype
  ...
  IF (sign(z_one,equilibrium_in(1)%global_param%psi_bound-equilibrium_in(1)%global_param%psi_ax)*isigma_Bp_in*sigma_Ip_in .LE. 0._R8) THEN
  ...
  ELSE
    ...
    IF (associated(equilibrium_in(1)%profiles_1d%pprime)) THEN
      pedge_rel=0._R8
      DO i=2,nb1D
        pedge_rel = pedge_rel + equilibrium_in(1)%profiles_1d%pprime(i) * (equilibrium_in(1)%profiles_1d%psi(i)-equilibrium_in(1)%profiles_1d%psi(i-1))
      END DO
  ...
  !
  call euitm_copy(equilibrium_in,equilibrium_out)
  !
  ...
  do it=1,size(equilibrium_out)
    ...
    equilibrium_out(it)%profiles_1d%pprime = sigma_Ip_eff * sigma_Bp_eff / TWOPI**exp_Bp_eff * equilibrium_out(it)%profiles_1d%pprime
    equilibrium_out(it)%profiles_1d%ffprime = sigma_Ip_eff * sigma_Bp_eff / TWOPI**exp_Bp_eff * equilibrium_out(it)%profiles_1d%ffprime
    ...
    ! profiles_2d:
    !
    equilibrium_out(it)%profiles_2d%psi = fact_psi * equilibrium_out(it)%profiles_2d%psi
    equilibrium_out(it)%profiles_2d%jphi = sigma_Ip_eff * equilibrium_out(it)%profiles_2d%jphi
    equilibrium_out(it)%profiles_2d%jpar = sigma_Ip_eff * equilibrium_out(it)%profiles_2d%jpar
    ...
    equilibrium_out(it)%profiles_2d%br = sigma_RphiZ_eff * sigma_Ip_eff * equilibrium_out(it)%profiles_2d%br
    equilibrium_out(it)%profiles_2d%bz = sigma_RphiZ_eff * sigma_Ip_eff * equilibrium_out(it)%profiles_2d%bz
    equilibrium_out(it)%profiles_2d%bphi = sigma_B0_eff * equilibrium_out(it)%profiles_2d%bphi
    equilibrium_out(it)%profiles_2d%vphi = sigma_RphiZ_eff * equilibrium_out(it)%profiles_2d%vphi
    equilibrium_out(it)%profiles_2d%vtheta = sigma_RphiZ_eff * sigma_rhothetaphi_eff * equilibrium_out(it)%profiles_2d%vtheta
    !
    ...
    IF (associated(equilibrium_out(it)%coord_sys%grid_type)) THEN
      IF (allocated(equilibrium_out(it)%coord_sys%grid_type)) THEN
        ...
        IF (i_gridtype.EQ.2) THEN
          ...
          equilibrium_out(it)%coord_sys%jacobian = fact_psi * sigma_rhothetaphi_eff * equilibrium_out(it)%coord_sys%jacobian
          equilibrium_out(it)%coord_sys%g_11 = fact_psi**2 * equilibrium_out(it)%coord_sys%g_11
          equilibrium_out(it)%coord_sys%g_12 = fact_psi * equilibrium_out(it)%coord_sys%g_12
          equilibrium_out(it)%coord_sys%g_13 = fact_psi * equilibrium_out(it)%coord_sys%g_13
        ELSE
          print *,'equilibrium_out(it)%coord_sys%grid_type(1) = ',equilibrium_out(it)%coord_sys%grid_type(1),' assumes no effect of COCOS required in CPOequilibrium%coord_sys'
        END IF
99      IF (i_gridtype .EQ. -99) THEN
        ...
        END IF
      ELSE
        print *,'equilibrium_out(it)%coord_sys%grid_type not ALLOCATED => assumes no effect of COCOS required in CPOequilibrium%coord_sys'
      END IF
    ELSE
      print *,'equilibrium_out(it)%coord_sys%grid_type not ASSOCIATED => assumes no effect of COCOS required in CPOequilibrium%coord_sys'
    END IF
    !
  END do
  !
