     ...
     ELSE IF (NFUNRHO.EQ.1 .AND. NRHOMESH.EQ.1) THEN
       ...
       !        GET VALUE OF RHOTOR FOR GIVEN S ARRAY
        ...
        if (TENSPROF .NE. 0) THEN
          call interpos(FCSM(1:NPPF1),RPPF(1:NPPF1),nin=NPPF1,nout=NISO,xout=CIDRTOR(1:NISO),tension=TENSPROF,YOUT=ZTEMP,nbc=(/2 , 2 /), &
            & ybc=(/ RPPF(1), RPPF(NPPF1) /) )
        else
          call interpos(FCSM(1:NPPF1),RPPF(1:NPPF1),nin=NPPF1,nout=NISO,xout=CIDRTOR(1:NISO),tension=TENSPROF,YOUT=ZTEMP,nbc=(/0 , 0 /), &
            & ybc=(/ 0._rkind, 0._rkind /) )
        end if
        ! Compute dp/dpsi_norm on ZS, use finite tension to be able to fix edge values
        call interpos(CSIPR(1:NISO)**2,ZTEMP(1:NISO),nin=NISO,nout=KN,xout=ZS(1:KN)**2,tension=-0.1_rkind,YOUTP=PT,nbc=(/2 , 2 /), &
            & ybc=(/ ZTEMP(1), ZTEMP(NISO) /) )
        PT = - PT / SPSIM
!!$        PRINT *,'NPPF+1,  FCSM(1:NPPF+1)',NPPF+1,FCSM(1:NPPF+1)
!!$        PRINT *,'NPPF+1, RPPF(1:NPPF+1)',NPPF+1,RPPF(1:NPPF+1)
!!$        PRINT *,'KN, PT(1:KN)',KN,PT(1:KN)
