...
!
  interface
     subroutine prof2d_rz_to_fluxtheta(RR,ZZ,f2darray_in,Rbnd,Zbnd,Raxis,Zaxis,flux_norm_out,theta_out,rhopolar_out,Rout,Zout,farray_out,tension_in)
!
       use itm_types
       use interpos_module
!
       IMPLICIT NONE
       real(R8), allocatable, intent(IN) :: RR(:), ZZ(:), f2darray_in(:,:,:), Rbnd(:), Zbnd(:), flux_norm_out(:), theta_out(:)
...
! datainfo
  ...
  eqchease_out(1)%codeparam%codeversion(1) = 'Id: $Id: metrictoitm_afterfpp.f90 826 2013-06-13 08:52:10Z osauter $ ; Global svnversion ' // "825M"
...
! profiles_1d
  eqchease_out(1)%profiles_1d%psi = SIGNIPXP * Ksigma_Bp * twopi**Kexp_Bp * eqchease_out(1)%profiles_1d%psi * R0EXP**2 * B0EXP
  ...
  eqchease_out(1)%profiles_1d%pprime = SIGNIPXP * Ksigma_Bp / twopi**Kexp_Bp * eqchease_out(1)%profiles_1d%pprime &
       & * B0EXP / ZMU0 / R0EXP**2
  eqchease_out(1)%profiles_1d%ffprime = SIGNIPXP * Ksigma_Bp / twopi**Kexp_Bp * eqchease_out(1)%profiles_1d%ffprime * B0EXP
  eqchease_out(1)%profiles_1d%jphi = SIGNIPXP * eqchease_out(1)%profiles_1d%jphi * B0EXP/zmu0/R0EXP
  eqchease_out(1)%profiles_1d%jparallel = SIGNIPXP * eqchease_out(1)%profiles_1d%jparallel * B0EXP/zmu0/R0EXP
  ...
  eqchease_out(1)%profiles_1d%dpsidrho_tor = SIGNIPXP*Ksigma_Bp*twopi**Kexp_Bp * eqchease_out(1)%profiles_1d%dpsidrho_tor &
       & * R0EXP*B0EXP
! eqchease_out(1)%profiles_1d%rho_vol is normalized
  ...
  eqchease_out(1)%profiles_1d%vprime = SIGNIPXP * Ksigma_Bp / twopi**Kexp_Bp * eqchease_out(1)%profiles_1d%vprime * R0EXP / B0EXP
! assume drho -> drho_tor not normalized, in [m]
  ...
  eqchease_out(1)%profiles_1d%aprime = SIGNIPXP * Ksigma_Bp / twopi**Kexp_Bp * eqchease_out(1)%profiles_1d%aprime / B0EXP
...
! global_param (after profiles_1d since related to it and needs to unnormalize only once)
  ...
  eqchease_out(1)%global_param%w_mhd = 1.5_rkind * CPBAR*B0EXP**2/zmu0 * eqchease_out(1)%global_param%volume/R0EXP**3
! profiles_2d
...
! note that box dimensions are already multiplied by R0EXP
  ...
  eqchease_out(1)%profiles_2d(1)%psi = SIGNIPXP * Ksigma_Bp * twopi**Kexp_Bp * EQDSPSI(1:NRBOX,1:NZBOX) * R0EXP**2 * B0EXP
! EQDSPSI is shifted to have psi_edge=0, so shift it to given psi_edge
  eqchease_out(1)%profiles_2d(1)%psi = eqchease_out(1)%profiles_2d(1)%psi + eqchease_out(1)%global_param%psi_bound
! BR = -1/R dpsi/dZ
! BZ = 1/R dpsi/dR
! Bphi = F_dia/R
  ...
  eqchease_out(1)%profiles_2d(1)%bphi(1:NRBOX,1:NZBOX) = SIGNB0XP * eqchease_out(1)%profiles_2d(1)%bphi(1:NRBOX,1:NZBOX) * B0EXP
...
!
      ...
      if (ii .EQ. 2) eqchease_out(1)%profiles_2d(ii)%grid_type(ii) = 'inverse, (psi,theta), COCOS=13'
      if (ii .EQ. 3) eqchease_out(1)%profiles_2d(ii)%grid_type(ii) = 'inverse, (rho_tor_norm,theta), COCOS=13'
...
!
      if (ii .EQ. 2) then
        ...
        eqchease_out(1)%profiles_2d(ii)%grid%dim1 = eqchease_out(1)%profiles_1d%psi
      elseif (ii .EQ. 3) then
        ndim1 = size(eqchease_out(1)%profiles_1d%rho_tor)
        allocate(eqchease_out(1)%profiles_2d(ii)%grid%dim1(ndim1))
        eqchease_out(1)%profiles_2d(ii)%grid%dim1 = eqchease_out(1)%profiles_1d%rho_tor / eqchease_out(1)%profiles_1d%rho_tor(ndim1)
      else
      ...
      eqchease_out(1)%profiles_2d(ii)%grid%dim2 = theta_out(:)
      allocate(eqchease_out(1)%profiles_2d(ii)%theta(ndim1,ndim2))
      do i=1,ndim2
        eqchease_out(1)%profiles_2d(ii)%theta(:,i) = eqchease_out(1)%profiles_2d(ii)%grid%dim2(i)
      end do
! All flux surface quantities in profiles_2d can now be copied from profiles_1d since they do not depend on theta
      ...
      do i=1,ndim1
        eqchease_out(1)%profiles_2d(ii)%psi(i,:) = eqchease_out(1)%profiles_1d%psi(i)
        eqchease_out(1)%profiles_2d(ii)%phi(i,:) = eqchease_out(1)%profiles_1d%phi(i)
        eqchease_out(1)%profiles_2d(ii)%pressure(i,:) = eqchease_out(1)%profiles_1d%pressure(i)
      end do
!
      ...
      elseif (mod(nprof2d,10) .EQ. 2) THEN
        ...
        write(6,*) 'mod(nprof2d,10) = ',mod(nprof2d,10),' should not happen in nprof2d part of metrictoitm, skip further part'
        call flush(6)
        go to 222
      end if
      allocate(zflux_norm(ndim1))
      allocate(f2darray_RZ(size(eqchease_out(1)%profiles_2d(1)%grid%dim1),size(eqchease_out(1)%profiles_2d(1)%grid%dim2),ININ))
      if (ii .eq. 2) THEN
! interpolate on normalized psi flux
        f2darray_RZ(:,:,1) = (eqchease_out(1)%profiles_2d(1)%psi(:,:)-eqchease_out(1)%profiles_2d(ii)%grid%dim1(1))/(eqchease_out(1)%profiles_2d(ii)%grid%dim1(ndim1)-eqchease_out(1)%profiles_2d(ii)%grid%dim1(1))
        zflux_norm = (eqchease_out(1)%profiles_2d(ii)%grid%dim1-eqchease_out(1)%profiles_2d(ii)%grid%dim1(1))/(eqchease_out(1)%profiles_2d(ii)%grid%dim1(ndim1)-eqchease_out(1)%profiles_2d(ii)%grid%dim1(1))
      else
...
! Thus gives the normalized flux here and then
      call prof2d_rz_to_fluxtheta(RR,ZZ,f2darray_RZ,Rbnd,Zbnd, &
        ...
        & zflux_norm,theta_out,rhopolar_out, &
        & Rout,Zout,f2darray_psitheta,tension_def)
!
      ...
      if (nverbose .ge. 1) write(6,*) 'profiles_2d(ii)%vtheta contains polar rho value(dim1,dim2) of each flux surface (dim1) at theta values (dim2)'
...
! XTOR uses equidistant polar theta and not J=c(psi) R^NER grad(psi)^NEGP jacobian
    write(eqchease_out(1)%coord_sys%grid_type(2),'(A,I2)') 'inverse, (psi,theta,phi), equidistant polar, cocos= ',COCOS_OUT
  ...
  elseif (NER.EQ.2 .AND. NEGP.EQ.0) THEN
    write(eqchease_out(1)%coord_sys%grid_type(2),'(A,I2)') 'inverse, (psi,chi,phi), straight field line, cocos= ',COCOS_OUT
    eqchease_out(1)%coord_sys%grid_type(3) = '1'
    eqchease_out(1)%coord_sys%grid_type(4) = 'straight field line, NER=2, NEGP=0, Jac=c(psi) R^NER grad(psi)^NEGP'
  elseif (NER.EQ.1 .AND. NEGP.EQ.-1) THEN
    write(eqchease_out(1)%coord_sys%grid_type(2),'(A,I2)') 'inverse, (psi,chi,phi), equal arc, cocos= ',COCOS_OUT
  ...
  elseif (NER.EQ.0 .AND. NEGP.EQ.0) THEN
    write(eqchease_out(1)%coord_sys%grid_type(2),'(A,I2)') 'inverse, (psi,chi,phi), Jac is psi function, cocos= ',COCOS_OUT
    eqchease_out(1)%coord_sys%grid_type(3) = '2'
    eqchease_out(1)%coord_sys%grid_type(4) = 'ala Hamada, NER=0, NEGP=0, Jac=c(psi) R^NER grad(psi)^NEGP'
  else
    write(eqchease_out(1)%coord_sys%grid_type(2),'(A,I2)') 'inverse, (psi,chi,phi), general chi, cocos= ',COCOS_OUT
  ...
  eqchease_out(1)%coord_sys%g_12 = SIGNIPXP * Ksigma_Bp * twopi**Kexp_Bp * eqchease_out(1)%coord_sys%g_12 * B0EXP
...
!
! D_I: IDEAL MERCIER
  eqchease_out(1)%profiles_1d%phi_flow = (/RC0P, SMERCI /)
! D_R: RESISTIVE INTERCHANGE
  eqchease_out(1)%profiles_1d%s_flow = (/RC0P, SMERCR /)
! NCBAL: Ballooning (0 if stable)
  eqchease_out(1)%profiles_1d%h_flow = real((/0, NCBAL /),rkind)
!
  return
