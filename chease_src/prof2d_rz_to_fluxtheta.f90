subroutine prof2d_rz_to_fluxtheta(RR,ZZ,f2darray_in,Rbnd,Zbnd,Raxis,Zaxis,flux_norm_out,theta_out,rhopolar_out,Rout,Zout,farray_out,tension_in)
  ...
  interface
     subroutine interpos2d_cartesian(RR,ZZ,f2darray_in,Rout,Zout,f2darray_out,tension_in,kextrapol)
       ...
       IMPLICIT NONE
       !
       real(R8), allocatable, intent(IN) :: RR(:), ZZ(:), f2darray_in(:,:,:), Rout(:,:), Zout(:,:)
  ...
  if ((nbnd_eff .GT. i) .AND. (NVERBOSE.GE.0)) write(6,*) &
    & 'Some points along plasma boundary too close in theta and ignored in prof2dRZ_to_fluxtheta: nbnd_eff=',nbnd_eff,' reduced to ',i
  nbnd_eff = i;
  ...
  !
  do j=1,ntheta
    call interpos(sqrt(f2darray_polar(:,j,1)),rhomesh(:,j),nflux_out,nout=nflux_out,tension=tension_eff,xout=sqrt(flux_norm_out),yout=rhopolar_out(:,j), &
      & nbc=(/2, 2/),ybc=(/rhomesh(1,j), rhomesh(nflux_out,j) /))
    Rout(:,j) = Raxis + rhopolar_out(:,j) * cos(theta_out(j))
    Zout(:,j) = Zaxis + rhopolar_out(:,j) * sin(theta_out(j))
    farray_out(:,j,1) = flux_norm_out;
    do k=2,nin
      call interpos(rhomesh(:,j),f2darray_polar(:,j,k),nflux_out,nout=nflux_out,tension=tension_eff,xout=rhopolar_out(:,j),yout=farray_out(:,j,k), &
        & nbc=(/2, 2/),ybc=(/f2darray_polar(1,j,k), f2darray_polar(nflux_out,j,k) /))
    end do
  end do
