...
!!$  end if
  ...
  ! find max_dimensions, note profiles_2d is an array in both itm and ids
  ndim1 = size(equil_in_itm(1)%profiles_2d(1)%grid%dim1)
  ndim2 = size(equil_in_itm(1)%profiles_2d(1)%grid%dim2)
  do it=2,nb_times
    do i=1,size(equil_in_itm(it)%profiles_2d)
      ndim1 = max(ndim1,size(equil_in_itm(it)%profiles_2d(i)%grid%dim1))
      ndim2 = max(ndim2,size(equil_in_itm(it)%profiles_2d(i)%grid%dim2))
    end do
  end do
  do i=1,size(equil_out_ids%Profiles_2D)
    allocate(equil_out_ids%Profiles_2D(i)%Grid%Dim1(ndim1,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%Grid%Dim2(ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%R(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%Z(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%Psi(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%Theta(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%Phi(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%J_Tor(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%J_Parallel(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%B_R(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%B_Z(ndim1,ndim2,nb_times))
    allocate(equil_out_ids%Profiles_2D(i)%B_Tor(ndim1,ndim2,nb_times))
  end do
  !
