SUBROUTINE IODISK(KOPT)
  ...
  REAL(RKIND), ALLOCATABLE :: ZSTEMP(:), FCSM_eff(:)
...
35 CONTINUE
  ...
  if (EQCHEASE_IN(1)%GLOBAL_PARAM%TOROID_FIELD%B0 .GT. -1e+40_rkind) THEN
    if (EQCHEASE_IN(1)%GLOBAL_PARAM%TOROID_FIELD%R0 .LE. -1e+40_rkind) then
      B0EXP = EQCHEASE_IN(1)%GLOBAL_PARAM%TOROID_FIELD%B0
    else
      B0EXP = EQCHEASE_IN(1)%GLOBAL_PARAM%TOROID_FIELD%R0 * EQCHEASE_IN(1)%GLOBAL_PARAM%TOROID_FIELD%B0 / R0EXP
    endif
  else
    IF (NVERBOSE .GE. 1) print *,'B0 not given in GLOBAL_PARAM%TOROID_FIELD%B0, take namelist value'
  endif
...
50 CONTINUE
  ...
  WRITE(NXTOR) (DPSI_NI(L),DPSI_TI(L),LAMBDA0(L),DLAMBDA(L), &
                SD_VMIN(L),SD_VMAX(L),L=1,NPOPULATIONS)
  !
  RETURN
  !
  !----------------------------------------------------------------------
