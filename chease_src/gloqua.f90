SUBROUTINE GLOQUA(PSM,KPSM,KCASE)
  ...
  eqchease_out(1)%profiles_1d%rho_tor(1:KPSM1) = sqrt(eqchease_out(1)%profiles_1d%phi(1:KPSM1)  / PI &
       & * eqchease_out_add_1d(KPSM1,iirgeo) / eqchease_out(1)%profiles_1d%f_dia(KPSM1))
  !dpsicheasedrhotor=B0*rhotor/q (to convert d/dpsi in dprime = d/drho_tor=d/dpsi * dpsi/drho_tor)
  eqchease_out(1)%profiles_1d%dpsidrho_tor(1:KPSM1) = eqchease_out(1)%profiles_1d%f_dia(KPSM1)/eqchease_out_add_1d(KPSM1,iirgeo) &
    & * eqchease_out(1)%profiles_1d%rho_tor(1:KPSM1) / eqchease_out(1)%profiles_1d%q(1:KPSM1)
  ...
  !
  CALL INTERPOS(eqchease_out(1)%profiles_1d%rho_vol,eqchease_out(1)%profiles_1d%q,KPSM1,TENSION=TENS_DEF, &
       & YOUT=ZDUMMY1(1:KPSM1),YOUTP=eqchease_out_add_1d(:,iishear), &
       & nbc=(/2, 2/), ybc=(/eqchease_out(1)%profiles_1d%q(1), eqchease_out(1)%profiles_1d%q(KPSM1)/))
  eqchease_out_add_1d(:,iishear) = eqchease_out(1)%profiles_1d%rho_vol * eqchease_out_add_1d(:,iishear) &
