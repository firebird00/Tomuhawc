SUBROUTINE SURF_METRICS_ONAXIS
  ...
  do ichi=1,NCHI
    ...
    ! on s so that extrapolation distance near center is small
    CALL INTERPOS(SMISOP1(2:NISO1EFF1), &
         & eqchease_out(1)%coord_sys%grid%dim1(2:NISO1EFF1)**2 * eqchease_out(1)%coord_sys%g_22(2:NISO1EFF1,ichi), &
         & NIN=NISO1EFF1-1,NOUT=NISO1EFF1,TENSION=10._rkind*TENS_DEF,XOUT=SMISOP1(1:NISO1EFF1), &
         & YOUT=eqchease_out(1)%coord_sys%g_13(1:NISO1EFF1,ichi), &
         & NBC=(/2, 2/), YBC=(/eqchease_out(1)%coord_sys%grid%dim1(2)**2 * eqchease_out(1)%coord_sys%g_22(2,ichi), &
         & eqchease_out(1)%coord_sys%grid%dim1(NISO1EFF1)**2 * eqchease_out(1)%coord_sys%g_22(NISO1EFF1,ichi)/))
    eqchease_out(1)%coord_sys%g_23(2:NISO1EFF1,ichi) = eqchease_out(1)%coord_sys%g_13(2:NISO1EFF1,ichi) / &
         & (eqchease_out(1)%coord_sys%grid%dim1(2:NISO1EFF1)**2)
    eqchease_out(1)%coord_sys%g_23(1,ichi) = 0._rkind
  end do
