...
1000 FORMAT(A)
  ...
  IF (NITMOPT.GT.0 .AND. MOD(NITMOPT,10) .EQ. 1) THEN
     ...
     !
     IF (NVERBOSE .GE. 2) PRINT *,'salut1, size(equil_in)= ',size(equil_in)
     ! allocate(eqchease_in(1))
     ! call euITM_copy_slice2slice_equilibrium(equil_in(2),eqchease_in(1))
     eqchease_in => equil_in
     IF (NVERBOSE .GE. 2) write(*,*) 'equil_in(1)%time = ',equil_in(1)%time
     ! write(*,*) 'eqchease_in(1)%eqgeometry%geom_axis%r = ',eqchease_in(1)%eqgeometry%geom_axis%r
     ! write(*,*) 'eqchease_in(1)%datainfo%comment(:) = ',eqchease_in(1)%datainfo%comment(:)
     if (associated(equil_in(1)%eqgeometry%boundary(1)%r)) then
        ! PRINT *,' equil_in(1)%eqgeometry%boundary(1)%r(1:2)= ',equil_in(1)%eqgeometry%boundary(1)%r(1:2)
     else
       IF (NVERBOSE .GE. 0) PRINT *,'equil_in(1)%eqgeometry%boundary(1)%r not associated'
       stop 'equil_in(1)%eqgeometry%boundary(1)%r'
     end if

     ! print *,'size(equil_in(1)%codeparam%parameters(:)) then equil_in(1)%codeparam%parameters(:)', &
     !      size(equil_in(1)%codeparam%parameters(:))
     ! print *,equil_in(1)%codeparam%parameters(:)
     ! print *,'associated(codeparam_param%parameters)= ',associated(codeparam_param%parameters)
     if (associated(codeparam_param%parameters)) then
...
!!$  allocate(eqchease_out(1))
!!$  allocate(eqchease_out(1)%profiles_2d(1))
!!$  allocate(eqchease_out(1)%eqgeometry%boundary(1))
!!$  if (associated(equil_out)) deallocate(equil_out)
!!$  allocate(equil_out(1))
!!$  allocate(equil_out(1)%profiles_2d(1))
!!$  allocate(equil_out(1)%eqgeometry%boundary(1))
  !
