...
70 CONTINUE
  ...
  CALL RVAR('TOROIDAL RHOTOR(EDGE)=sqrt(PHI/PI/(T(edge)/Rgeom(edge)))   ', &
       & sqrt(CTORSRF*eqchease_out_add_1d(NISO1EFF1,iirgeo)/TMF(NISO1EFF)/CPI))
...
!!$  ENDIF
  ...
  CALL RARRAY('Int(R dlp/|grad(psi)|)=Int(J dchi)',(/ RC0P, TWOPI*RJ5(1:NISO1EFF) /),NISO1EFF1)
  ...
  XX=SMISOP1(2:5)
  YY=RJ7s(2:5,5)/RJ5(2:5)
  CALL RARRAY('<R**2>',(/ FIT0CUB(RC0P), RJ7s(1:NISO1EFF,5)/RJ5(1:NISO1EFF) /),NISO1EFF1)
