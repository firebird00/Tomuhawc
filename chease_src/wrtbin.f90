SUBROUTINE WRTBIN
  ...
  call binwrtM(mp,'SPITZER',1,1,SPITZER)
  ...
  call binwrtM(mp,'DLAMBDA',size(DLAMBDA),1,DLAMBDA)
  call binwrtM(mp,'LAMBDA0',size(LAMBDA0),1,LAMBDA0)
  call binwrtM(mp,'DPSI_NI',size(DPSI_NI),1,DPSI_NI)
  call binwrtM(mp,'DPSI_TI',size(DPSI_TI),1,DPSI_TI)
  call binwrtM(mp,'SD_VMIN',size(SD_VMIN),1,SD_VMIN)
  call binwrtM(mp,'SD_VMAX',size(SD_VMAX),1,SD_VMAX)
  ...
  !
  aux(:) = (/ (ZRSUR(i) * COS(ZTSUR(J283)), i=1,INSUR) /)
  call binwrtM(mp,'ZRSUR',INSUR,1,aux)
  !
  aux(:) = (/ (ZRSUR(i) * SIN(ZTSUR(J283)), i=1,INSUR) /)
  ...
  deallocate(aux)
  !
  allocate(aux(NISO1EFF),stat=state)
