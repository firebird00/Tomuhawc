...
!!$     if ( associated(equil_in_ids)) then
        ! assume equil_in_ids already loaded
!!$     else
...
!     end if
     !
!!$     IF (NVERBOSE .GE. 2) PRINT *,'salut1, size(equil_in_ids)= ',size(equil_in_ids)
     IF (NVERBOSE .GE. 2) write(*,*) 'equil_in_ids%timebase = ',equil_in_ids%timebase
     !
  else
    IF (NVERBOSE .GE. 1) PRINT *,'equilibrium not taken from ITM structure, thus equil_in_ids not allocated'
  end IF
  !
